-- Fetch migration 0017: grade brand consistency against the provider's own
-- name, not our synthesized display name.
--
-- The OSM importer builds a display name from our brand when the POI carries a
-- `branch` tag: "Giant Food — Brandywine Bikes". That is good for display, but
-- it meant the brand guard was checking a string we had just stamped our own
-- brand onto, so every mis-tagged POI with a branch tag passed automatically.
-- Brandywine Bikes and Cycling World are bicycle shops; they were listed as
-- grocery stores.
--
-- Root cause was upstream: the giant-food brand list included the bare word
-- "Giant", which in OpenStreetMap is the bicycle manufacturer (and an
-- unrelated fuel chain). That is fixed in _shared/osm-directory.ts. This
-- migration closes the laundering hole itself so the next mis-tagged brand
-- cannot repeat it.
--
-- Note on Pennsylvania: the ~124 "Giant" stores in PA are NOT errors. GIANT
-- is Ahold Delhaize's Pennsylvania grocery banner, a sister chain to Giant
-- Food. They stay.

alter table stores
  add column source_name text;

comment on column stores.source_name is
  'The provider''s own name for this place, before any display formatting. '
  'Brand-consistency grading uses this, never the synthesized display name.';

-- ---------------------------------------------------------------------------
-- Importer grades on the provider name when one is supplied.
-- ---------------------------------------------------------------------------
...
begin
  ...
  for v_row in select * from jsonb_array_elements(p_rows) loop
    ...
    -- Grade the provider's name. Falling back to the display name only when
    -- the provider gave us nothing of its own.
...
-- ---------------------------------------------------------------------------
-- Clean up what the bare-"Giant" match already let in.
--
-- Giant Food and its sister banner GIANT operate in DC, MD, VA, DE, PA, NJ,
-- NY and WV. Anything filed under this retailer outside that footprint came
-- from the bicycle or fuel brand, not from a grocery chain.
-- ---------------------------------------------------------------------------
update stores s
set review_status = 'REJECTED',
    review_reason = 'Matched the bicycle/fuel brand "Giant", not the grocery banner'
from retailers r
where r.id = s.retailer_id
  and r.slug = 'giant-food'
  and s.review_status = 'OK'
  and (
    s.state not in ('DC', 'MD', 'VA', 'DE', 'PA', 'NJ', 'NY', 'WV')
    or s.name ~* '(bike|bicycle|cycling|cyclery)'
  );
